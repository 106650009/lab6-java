...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Data[] tranData=new Data[transNameArray.length];
        for(int i=0;i<tranData.length;i++){
            tranData[i]=new Data();
            tranData[i].name=transNameArray[i];
            tranData[i].photo=transPhotoIdArray[i];
        }
        MyAdapter transAdapter=new MyAdapter(tranData,R.layout.trans_list);
